...
function parallaxsome_scripts() {
    ...
    wp_enqueue_style('parallaxsome-responsive',get_template_directory_uri().'/assets/css/keyboard.css');
		...
		foreach ( $parallaxsome_single_menu_fields as $section_id => $section_value ) {
			...
			if( !empty( $parallaxsome_menu_title ) ) {
                ...
                if( $parallax_menu_type == 'float' ) {
                	...
                	<div class="px-tooltip"><?php echo esc_html( $parallaxsome_menu_title ); ?></div>
                	<?php
                } else {
 		...
 		<div class="ps-container">
	 		<header class="entry-header">
				...
				<?php
					...
					$breadcrumb_op = get_theme_mod('parallaxsome_breadcrumbs_options','show');
					if($breadcrumb_op == 'show') {
